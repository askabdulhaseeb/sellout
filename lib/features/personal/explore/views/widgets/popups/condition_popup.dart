...
class ConditionTypeFilterDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Consumer<ExploreProvider>(
      builder: (BuildContext context, ExploreProvider provider, Widget? child) {
        return AlertDialog(
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: ConditionType.values.map((ConditionType condition) {
              return ListTile(
                title: Text(condition.code.toUpperCase()),  // Using code from ConditionType
                onTap: () {
                  provider.setConditionType(condition);  // Set the selected condition type
