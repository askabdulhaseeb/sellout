...
class CreateChatGroupProvider with ChangeNotifier {
 ...
 Future<void> setImages(
  ...
  final List<PickedAttachment> selectedMedia =
      _attachments.where((PickedAttachment element) => element.selectedMedia != null).toList();
    ...
    MaterialPageRoute<List<PickedAttachment>>(builder: (_) {
      return PickableAttachmentScreen(
        option: PickableAttachmentOption(
          ...
          type: type,
          selectedMedia:
              selectedMedia.map((PickedAttachment e) => e.selectedMedia!).toList(),
        ),
      );
    }),
  );
...
Future<void> createChat(BuildContext context) async {
  ...
  try {
    ...
    final DataState<ChatEntity> result = await _createChatUsecase.call(params);
    if (result is DataSuccess<ChatEntity>) {    setLoading(false);

      AppNavigator.pushNamedAndRemoveUntil(DashboardScreen.routeName , (_) => false,);  
      }
    ...
    AppLog.error(
      'Exception occurred while creating group chat',
