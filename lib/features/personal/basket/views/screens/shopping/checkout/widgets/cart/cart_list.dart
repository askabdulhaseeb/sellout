...
class _CartItemsListState extends State<CartItemsList> {
  late Future<List<SellerGroup>> _groupedFuture;
  ...
  Future<List<SellerGroup>> _fetchAndGroup() async {
    ...
    List<SellerGroup> groups = <SellerGroup>[];
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<List<SellerGroup>>(
      ...
      builder:
          (BuildContext context, AsyncSnapshot<List<SellerGroup>> snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              return const Center(child: CircularProgressIndicator());
            }

            final List<SellerGroup> groups = snapshot.data ?? <SellerGroup>[];
            if (groups.isEmpty) {
              return Center(
                child: Text(
                  'Your cart is empty',
